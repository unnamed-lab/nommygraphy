...
export function BlogSidebar() {
  return (
    <div className="space-y-8">
      ...
      <AdUnit adSlot="1357908642" format="rectangle" responsive={false} />
      ...
      <div className="bg-muted/30 p-6 rounded-xl">
        ...
        <div className="space-y-4">
          {popularPosts.map((post) => (
            <div key={post.slug}>
              <Link href={post.slug} className="text-sm font-medium hover:text-primary transition-colors">
                {post.title}
              </Link>
      ...
      <AdUnit adSlot="9753124680" format="rectangle" responsive={false} className="mt-8" />
    </div>
  )
}
